...
def score_ligand_batch(params, ligand_batch, model_binaries):
    ...
    # score the features for each model in the list and add dataframe of results to list
    for model in models:
        model_results.append(score(model, multi_pose_features))
        logging.info(f'Done scoring with {model[0]}')
    ...
    autoencoder = tf.keras.models.load_model("./autoencoder_15.h5")

    # encode and decode the features using the autoencoder
    autoencoder_x = merged_results[['xgboost_model', 
                        'ff_nn_1', 'ff_nn_2', 'ff_nn_3', 'ff_nn_4', 'ff_nn_5', 'ff_nn_6', 'ff_nn_7', 'ff_nn_8', 'ff_nn_9', 'ff_nn_10', 'ff_nn_11', 'ff_nn_12', 'ff_nn_13', 'ff_nn_14', 'ff_nn_15',
                        'wd_nn_1', 'wd_nn_2', 'wd_nn_3', 'wd_nn_4', 'wd_nn_5', 'wd_nn_6', 'wd_nn_7', 'wd_nn_8', 'wd_nn_9', 'wd_nn_10', 'wd_nn_11', 'wd_nn_12', 'wd_nn_13', 'wd_nn_14', 'wd_nn_15']].values
    encoded_features = autoencoder.predict(autoencoder_x)
    decoded_features = autoencoder.predict(encoded_features)

    # load the pre-trained model
    ensemble_model = tf.keras.models.load_model("ffnn_15.h5")

    # make predictions using the pre-trained model
    X = decoded_features
    y_pred = ensemble_model.predict(X)

    # create main scorch score using model predictions
    merged_results['SCORCH_pose_score'] = y_pred

    # calculate scorch certainty score
    max_std = 0.4714 # result from [0, 0, 1] or [1, 1, 0]
    minimum_val = 1 - max_std
    merged_results['SCORCH_stdev'] = merged_results[['xgboost_model', 'ff_nn_models_average', 'wd_nn_models_average']].std(axis=1, ddof=0)
